...
@section('content')
    ...
    <form method="POST" action="{{ route('user.document.update', array($userDocument->pivot->id))}}"
          class="form-horizontal row-fluid" enctype="multipart/form-data">
        @csrf
                ...
                <select autocomplete="document_id" name="document_id" class="span8" id="document_id">
                    ...
                    @foreach($documents as $document)
                        <option value="{{$document->id}}"
                                @if($userDocument->pivot->document_id==$document->id) selected @endif>{{$document->document_name}}</option>
                    @endforeach
                </select>
        ...
        @if($userDocument->pivot->document_url !="")
            <div class="control-group">
                <label class="control-label" for="document_url">Current Document</label>

                <div class="controls">
                    <a href="{{asset($userDocument->pivot->document_url)}}">{{$userDocument->ext}} Document</a>
                </div>
            </div>
        @endif
        <div class="control-group">
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <input class="span8" placeholder="Note" id="note" type="text"
                       ...
                       value="{{$userDocument->pivot->note}}" autocomplete="document_note" autofocus>
                @error('note')
                <span class="help-inline" role="alert">
                              <strong class="text-danger">{{ $message }}</strong>
                            </span>
